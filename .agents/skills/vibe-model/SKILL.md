---
name: vibe-model
description: Preview-first reconstruction of reference-driven hard-surface props, machinery, signage, modular architecture, and hero assemblies as direct Three.js source. Use when building, revising, visually matching, previewing, or preparing a procedural hard-surface model for a Vibe3D registry, or when improving, auditing, or upgrading the visual quality of an existing procedural Three.js model in any codebase, with or without a reference image or registry. Do not use for generic Three.js application code, bitmap editing, characters, organic assets, foliage, terrain, or particle effects.
---
...
When the model already exists — improving, auditing, or upgrading a
procedural model in a host codebase rather than building one from reference —
also read [improving-existing.md](references/improving-existing.md). It keeps
the same loop but replaces the registry, reference image, and preview CLI
with the host's own contract, a baseline capture, and an authored brief. The
Author, Preview, and Deliver sections below describe the registry workflow;
improving-existing.md states which parts transfer and what replaces the rest.
...
When the host renders below critic resolution, score that runtime capture
too (improving-existing.md). Apply the highest-impact correction and capture
again.
...
better reference evidence. A runtime-resolution miss of the primary read is
a massing/value problem, not an emissive problem.
