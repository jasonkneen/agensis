...
5. Assume winding changes when a ring insets. For a suspect cap or inward rim,
   ...
   normal.
   Pitched thin slabs (roofs, awnings, gable caps, hoods, canopies): `rotateX`
   on a BoxGeometry often swaps which edge is high *and* which face is
   FrontSide. A gable that reads as a hanging V, a dark soffit aimed at the
   camera, or sky showing through where shingles should be, is inverted pitch
   — not lighting. Do not set `side = DoubleSide` to hide it.
   Gate before accepting a capture: ridge world-Y > eave world-Y; front gable
   mean normal +Z, back −Z. For slabs whose ridge is toward z = 0,
   `rotation.x = (position.z < 0 ? -1 : 1) * pitch` lifts the ridge. Flip the
   rotation (or the out-axis scale) and recompute normals.
...
13. Treat critic feedback as evidence, not a numeric gradient. Prefer changes
    ...
    Author-as-critic scores are a note of intent, not acceptance.
...
18. Survive the host's runtime resolution. If the host downsamples, grade, or
    fogs the frame, score that frame. A secondary mass whose albedo sits at or
    below the crushed-black floor is not a mass; lift its value or enlarge its
    silhouette instead of adding lights.
19. Split the two brightest features in hue and value. Matching warm (or
    matching cool) lights merge into one smear when downsampled.
20. Keep light accents on an existing mass: a band, lip, ring, or inset that
    follows the host part. A camera-facing emissive slab that only reads as a
    light is a failed accent — shrink it onto the part or reject it. Do not
    use DoubleSide, extra glow, or a new quad to fake a missing landmark.
