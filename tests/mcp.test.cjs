...
const WS = 'ws-1';
const OTHER_WS = 'ws-2';
const AGENT = { agentId: 'agent-1', workspaceId: WS, name: 'Coder', handle: 'coder', agent: { model: 'claude-opus-4-8', description: 'coding agent' } };
...
function makeDeps(overrides = {}) {
  ...
  const deps = {
    getDb: () => overrides.db,
    verifyAgentConnectToken: async (token) => (token === 'good-token' ? AGENT : null),
