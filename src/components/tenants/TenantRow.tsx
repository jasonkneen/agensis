...
// ---------------------------------------------------------------------------
// One account. Deliberately the SAME row as the inbox's — three lines of text
// against a 32px tile, ~96px tall, with nothing drawn between rows:
//
//   [JK]  Jason Kneen                              15 Jan 2026
//         jason@bouncingfish.com
//         3 workspaces · 1 shared
//
...
export const TenantRow = React.memo(function TenantRow({ row, selected, onSelect }: TenantRowProps) {
  const { account, title, subtitle, summary, joined, initials, color } = row;
  // The email is the title when there is no display name; repeating it on line 2
  // would be two identical lines, so line 2 goes quiet instead.
  const showEmail = subtitle && subtitle !== title;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        aria-label={`Open ${title}${showEmail ? ` (${subtitle})` : ''}`}
        className={cn(
      ...
      <div className={cn('pointer-events-none relative z-10 flex min-w-0 items-start gap-2.5', ROW_PADDING)}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={cn('mt-1.5 truncate leading-snug text-muted-foreground', TEXT_BODY)}>
            {summary}
          </p>
        </div>
      </div>
