...
// Insert-or-freshness-merge an incoming object into a list, used by the realtime
// broadcast upsert handler. Shares preferFresherCanvasObject with the DB UPDATE
// path so an older out-of-order echo never reverts a newer local object.
export function upsertCanvasObject(
 ...
 const next = [...list];
 next[index] = preferFresherCanvasObject(next[index], incoming);
 return next;
}
  ...
  const channel = backendClient
   ...
   .on(
    ...
    ({ payload }: BroadcastPayload<BroadcastCanvasObjectPayload>) => {
     ...
     setObjects(prev => upsertCanvasObject(prev, normalized));
    }
   )
   .on(
    'broadcast',
