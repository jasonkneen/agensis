...
function getDefaultRestoreSize(type: FloatingWindowType): { width: number; height: number } {
  ...
  const sizeMap: Record<string, { width: number; height: number }> = {
    ...
    skills: { width: Math.min(640, Math.max(440, Math.round(viewport.width * 0.5))), height: Math.min(700, Math.max(520, Math.round(viewport.height * 0.72))) },
    ...
    resources: { width: Math.min(980, Math.max(720, Math.round(viewport.width * 0.72))), height: Math.min(760, Math.max(560, Math.round(viewport.height * 0.76))) },
