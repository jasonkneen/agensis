...
import { clampPaneSplit, type PaneSplitBounds } from './paneSplit';
...
//
// The clamp itself now lives in lib/paneSplit.ts, shared with the Memory
// window's files/preview split — the rule that a stored divider position must
// be re-derived from the live container is not specific to this window, and a
// second copy of it is a second place for it to drift. What stays here is what
// IS specific: the two floors and the default share.
// ---------------------------------------------------------------------------
...
export const AGENT_SPLIT_BOUNDS: PaneSplitBounds = {
  minLeading: AGENT_SPLIT_MIN_GRID_PX,
  minTrailing: AGENT_SPLIT_MIN_MAP_PX,
 ...
 * the map simply takes the remainder). See `clampPaneSplit` for the rules; this
 * is that function with the grid/map floors filled in.
 */
export function agentSplitGridHeight(stored: number | null, containerPx: number): number {
  return clampPaneSplit(stored, containerPx, AGENT_SPLIT_BOUNDS);
}
