 ...
 *
 * Deliberately NOT "connected" alone. Three states have to keep the panel up:
 ...
 * the audio of, and an error, which is the one moment the user most needs
 * something on screen to read. A panel that vanishes on failure leaves no trace
 * of what went wrong.
 */
export function shouldShowHuddleDock(input: HuddleDockVisibilityInput): boolean {
  if (!input.hasTarget) return false;
  return input.connected || input.live || input.hasError;
}
